...
class ChinaUnionPay
{
	public function makePurchase($org, $user, $purchase, $donationId, $simulate, &$msg) {
		...
		if ($org->test()) {
			$url = "http://beta-pos-cupapi.pvbcard.com/WSJCBAcqAPI.asmx";
		} else {
			$url = "https://cupacqapi.pvbcard.com/WSCUPAcqAPI.asmx";
		}
		...
		$client = new SoapClient($url,
			...
			"cache_wsdl" => 0));
		$response = $client->SaleTransaction(array ("pCommandString" => $this->makeCommandString($org, $user, $purchase, $donationId)));
	...
	private function makeCommandString($org, $user, $purchase, $donationId) {
		...
		"FIELD=215&VALUE=" . $donationId . ";" . 		// 215 Merchant Invoice Yes M(50) The invoice number of the merchant. Should be unique on merchant side.
		"FIELD=220&VALUE=" . date('mdY') . ";" . 		// 220 Merchant Invoice Date Yes A(10) The date for the merchant invoice issued. Format must be MMddyyyy
		"FIELD=223&VALUE=" . $_SERVER['REMOTE_ADDR'] . ";" . 	// 223 Client IP Address Yes IP The IP address of the client browser. Can be in IPv4 or IPv6 format.
		"FIELD=226&VALUE=" . $user->name() ;       		// 226 Cardholder Name Yes A(30) The name of the cardholder, as printed on the card	
	}
